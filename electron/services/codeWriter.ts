...
export class CodeWriter {
     ...
     */
    private extractCSSPropertyValue(css: string, selector: string, cssProp: string, mediaQuery?: string): string | null {
     ...
     */
    private patchCSSProperty(css: string, selector: string, prop: string, value: string, mediaQuery?: string): string {
        ...
        try {
            ...
            root.walkRules((rule) => {
                ...
                if (ruleSelector !== targetSelector) return;

                // E5: If mediaQuery is specified, verify the rule is inside the correct @media block
                if (mediaQuery) {
                    const parent = rule.parent;
                    if (!parent || parent.type !== 'atrule' || (parent as postcss.AtRule).name !== 'media') return;
                    const parentParams = `@media ${(parent as postcss.AtRule).params}`.trim();
                    if (!parentParams.includes(mediaQuery.replace('@media ', '').trim())) return;
                }

                // Found the rule — update or add the property
                let existingDecl: postcss.Declaration | null = null;
                rule.walkDecls((decl) => {
                ...
                if (existingDecl) {
                    (existingDecl as postcss.Declaration).value = value;
                } else {
            ...
            if (!found) {
                ...
                if (mediaQuery) {
                    let mediaBlock: postcss.AtRule | null = null;
                    ...
                    if (mediaBlock) {
                        ...
                        (mediaBlock as postcss.AtRule).append(newRule);
                    } else {
     ...
     */
    private patchCSSPropertyRegex(css: string, selector: string, prop: string, value: string, mediaQuery?: string): string {
        const cssProp = prop.replace(/([A-Z])/g, '-$1').toLowerCase();
