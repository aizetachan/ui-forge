...
import type { ComponentVariant, ComponentPropDef, StoryVariant } from './forgecoreTypes.js';
import type {
    ForgecoreManifest,
    ForgecoreComponent,
    ForgecoreRuntime,
    ...
    ParsedToken,
    ParsedRepoContents,
} from './forgecoreTypes.js';

// Re-export types that consumers of repoParser may need
export type {
    ForgecoreManifest,
    ForgecoreComponent,
    ForgecoreRuntime,
...
export class RepoParser {
     ...
     */
    private async loadManifest(repoPath: string): Promise<ForgecoreManifest | null> {
        const manifestPath = path.join(repoPath, 'forgecore.json');

        try {
            const content = await fs.readFile(manifestPath, 'utf-8');
            const manifest = JSON.parse(content) as ForgecoreManifest;
            return manifest;
        } catch {
            return null;
        }
     ...
     */
    private async parseComponentsFromManifest(repoPath: string, manifest: ForgecoreManifest): Promise<ParsedComponent[]> {
     ...
     */
    private async parseComponentFromManifestEntry(
        repoPath: string,
        name: string,
        config: ForgecoreComponent,
        manifest: ForgecoreManifest
        ...
        if (config.defaultProps) {
            ...
            for (const [propName, propValue] of Object.entries(config.defaultProps)) {
                ...
                if (SKIP_PROPS.includes(propName)) continue;
        ...
        {
            let tsPropDefs: ComponentPropDef[] = [];
            ...
            for (const tsPd of tsPropDefs) {
                if (existingPropNames.has(tsPd.name)) continue;
                if (SKIP_PROPS.includes(tsPd.name)) continue;
                propDefs.push(tsPd);
                mergedCount++;
            }
     ...
     */
    private inferPropTypeFromValue(propName: string, value: any): { type: string; control?: ComponentPropDef['control']; options?: string[] } {
     ...
     */
    private async parseTokensFromManifest(repoPath: string, manifest: ForgecoreManifest): Promise<ParsedToken[]> {
     ...
     */
    private extractCSSVariables(content: string, categories?: Record<string, string>): ParsedToken[] {
        const tokens: ParsedToken[] = [];
        ...
        while ((match = varRegex.exec(content)) !== null) {
            ...
            let type: ParsedToken['type'] = 'color';

            if (categories) {
                for (const [cat, prefix] of Object.entries(categories)) {
                    if (name.startsWith(prefix)) {
                        type = cat as ParsedToken['type'];
                        break;
                    }
                }
            } else {
     ...
     */
    private async loadThemeCSSFromManifest(repoPath: string, manifest: ForgecoreManifest): Promise<string | undefined> {
        // Get CSS paths from tokens.css OR runtime.globalCss (new format)
        const cssPaths = manifest.tokens?.css || manifest.runtime?.globalCss || [];

        if (cssPaths.length === 0) return undefined;
     ...
     */
    private async loadThemeCSS(repoPath: string): Promise<string | undefined> {
     ...
     */
    private async parseComponents(repoPath: string): Promise<ParsedComponent[]> {
        const components: ParsedComponent[] = [];
     ...
     */
    private async parseComponentFile(content: string, fileName: string, fullPath: string, repoPath: string): Promise<ParsedComponent | null> {
     ...
     */
    private findDestructuredProps(content: string): string | null {
     ...
     */
    private extractPropDefinitions(content: string, componentName: string): ComponentPropDef[] {
        ...
        if (startMatch) {
            ...
            for (const line of lines) {
                ...
                if (match) {
                    ...
                    if (defaultValue === undefined && type === 'boolean') defaultValue = false;
     ...
     */
    private async extractCssModuleInfo(content: string, tsxPath: string, repoPath: string): Promise<{
        baseClasses: string;
     ...
     */
    private extractBaseClassesFromCss(cssContent: string): string {
     ...
     */
    private extractVariantsFromCss(cssContent: string): ComponentVariant[] {
        const variants: ComponentVariant[] = [];
     ...
     */
    private transformCssModuleToGlobal(cssContent: string, componentName: string): string {
     ...
     */
    private cssPropertiesToTailwind(cssProperties: string): string {
     ...
     */
    private detectPrimaryTag(content: string): string {
     ...
     */
    private extractTextContent(content: string, componentName: string): string {
     ...
     */
    private extractDependencies(content: string, repoPath: string): string[] {
     ...
     */
    private async parseStoryVariants(componentPath: string): Promise<StoryVariant[]> {
     ...
     */
    private async parseTokens(repoPath: string): Promise<ParsedToken[]> {
        const tokens: ParsedToken[] = [];
     ...
     */
    private async parseThemeCss(repoPath: string): Promise<ParsedToken[]> {
        const tokens: ParsedToken[] = [];
     ...
     */
    private async parseJsonTokens(repoPath: string): Promise<ParsedToken[]> {
        const tokens: ParsedToken[] = [];
     ...
     */
    private parseJsonTokensRecursive(obj: any, prefix: string, tokens: ParsedToken[]): void {
     ...
     */
    private async parseCssTokens(repoPath: string): Promise<ParsedToken[]> {
        const tokens: ParsedToken[] = [];
     ...
     */
    private inferTokenType(name: string, value: string): 'color' | 'spacing' | 'typography' | 'radius' {
     ...
     */
    private async collectAssets(repoPath: string): Promise<ParsedRepoContents['repoAssets']> {
        const icons: Array<{ name: string; svgContent: string; relativePath: string }> = [];
        const images: Array<{ name: string; relativePath: string; mimeType: string }> = [];
        ...
        for (const dir of iconDirs) {
            try {
                ...
                if (icons.length > 0 && iconPaths.length === 0) break;
            } catch {
        ...
        for (const dir of imgDirs) {
            try {
                ...
                if (images.length > 0 && imagePaths.length === 0) break;
            } catch {
                // Directory not found, continue
            }
        }
