 ...
 */
export async function extractPropDefsFromAST(filePath, componentName, repoPath) {
    const propDefs = [];
    try {
        ...
        for (const prop of properties) {
            ...
            if (!isOwnProp && name.startsWith('on') && name.length > 2)
                continue;
 ...
 */
function findPropsTypeFromComponent(sourceFile, componentName) {
        ...
        if (match) {
            ...
            while ((assignMatch = assignRegex.exec(destructuredBlock)) !== null) {
                ...
                if (rawValue === 'true')
                    ...
                    defaults.set(propName, parseInt(rawValue));
                else if (/^\d+\.\d+$/.test(rawValue))
                    defaults.set(propName, parseFloat(rawValue));
 ...
 */
function resolveType(type, propName) {
    // Boolean
    ...
    if (type.isUnion()) {
        ...
        if (stringLiterals.length === meaningful.length && stringLiterals.length > 0) {
            const options = stringLiterals.map(t => t.getLiteralValue());
