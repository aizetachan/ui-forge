...
ipcMain.handle('git:clone', async (_event, repoUrl) => {
    try {
...
ipcMain.handle('git:commit-push', async (_event, repoPath, message, branch) => {
    try {
...
ipcMain.handle('git:fetch-status', async (_event, repoPath) => {
    try {
...
ipcMain.handle('git:get-branches', async (_event, repoPath) => {
...
ipcMain.handle('git:switch-branch', async (_event, repoPath, branchName) => {
    try {
...
ipcMain.handle('repo:parse', async (_event, repoPath) => {
    try {
...
ipcMain.handle('repo:read-file', async (_event, filePath) => {
    try {
